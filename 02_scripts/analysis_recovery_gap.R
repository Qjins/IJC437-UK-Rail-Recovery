...
#   03_figures/fig2_recovery_gap.png
#
...
# ------------------------------------------------------------
# 6. Visualisation: Recovery gap by region (Figure 2)
# ------------------------------------------------------------
p <- ggplot(
  recovery_gap,
  ...
  labs(
    ...
    x = "Region",
    y = "Recovery gap (Index points relative to 2019 baseline; 0 = full recovery)"
  ) +
  theme_minimal() +
  theme(
...
ggsave(
  filename = "03_figures/fig2_recovery_gap.png",
  plot = p,
  width = 8,
  height = 5
)
